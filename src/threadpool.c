...
static int threadpool_queue_enqueue(struct threadpool_queue *queue, void *data)
{
	if (queue->num_of_cells == THREAD_POOL_QUEUE_SIZE) {
		DEBUG("The queue is full, unable to add data to it.");
		return -1;
	}

	if (queue->cells[queue->tail] != NULL) {
		DEBUG("A problem was detected in the queue (expected NULL, but found a different value).");
		return -1;
	}
...
static void *threadpool_queue_dequeue(struct threadpool_queue *queue)
{
	...
	if (queue->num_of_cells == 0) {
			DEBUG("Tried to dequeue from an empty queue.");
			return NULL;
	}
...
static struct threadpool_task* threadpool_task_get_task(struct threadpool *pool)
{
	...
	if (pthread_mutex_lock(&(pool->mutex))) {
		log_error(errno, "pthread_mutex_lock");
		return NULL;
	}
	...
	while (threadpool_queue_is_empty(&(pool->tasks_queue)) && !pool->stop_flag) {
		...
		if (pthread_cond_wait(&(pool->cond),&(pool->mutex))) {
			log_error(errno, "pthread_cond_wait");
			if (pthread_mutex_unlock(&(pool->mutex))) {
				log_error(errno, "pthread_mutex_unlock");
			}
	...
	if ((task = (struct threadpool_task*)threadpool_queue_dequeue(&(pool->tasks_queue))) == NULL) {
		...
		DEBUG("Failed to obtain a task from the jobs queue.");
	}
	...
	while (1) {
		...
		if (task == NULL) {
			if (pool->stop_flag) {
				...
				DEBUG("Warning an error has occurred when trying to obtain a worker task.");
				DEBUG("The worker thread has exited.");
				break;
			}
		}
		...
		if (task->routine_cb) {
			...
			if (threadpool_queue_enqueue(&(pool->free_tasks_queue),task)) {
				DEBUG("Failed to enqueue a task to free tasks queue.");
			...
			if (threadpool_queue_getsize(&(pool->free_tasks_queue)) == 1) {
				...
				if (pthread_cond_broadcast(&(pool->free_tasks_cond))) {
					log_error(errno, "pthread_cond_broadcast");
					if (pthread_mutex_unlock(&(pool->free_tasks_mutex))) {
						log_error(errno, "pthread_mutex_unlock");
					}
...
static void *stop_worker_thr_routines_cb(void *ptr)
{
	...
	if (pthread_mutex_lock(&(pool->mutex))) {
		log_error(errno, "pthread_mutex_lock");
		DEBUG("Warning: Memory was not released.");
		DEBUG("Warning: Some of the worker threads may have failed to exit.");
		return NULL;
	}
...
struct threadpool* threadpool_init(int num_of_threads)
{
	...
	for (i = 0; i < THREAD_POOL_QUEUE_SIZE; i++) {
		...
		if (threadpool_queue_enqueue(&(pool->free_tasks_queue),(pool->tasks) + i)) {
			DEBUG("Failed to a task to the free tasks queue during initialization.");
			free(pool);
			return NULL;
		}
	}
	...
	/* Create the thr_arr. */
	pool->thr_arr = xmalloc(sizeof(pthread_t) * num_of_threads);
	...
	for (pool->num_of_threads = 0; pool->num_of_threads < num_of_threads; (pool->num_of_threads)++) {
		if (pthread_create(&(pool->thr_arr[pool->num_of_threads]),NULL,worker_thr_routine,pool)) {
			log_error(errno, "pthread_create");
...
int threadpool_add_task(struct threadpool *pool, void (*routine)(void*), void *data, int blocking)
{
	...
	if (pool == NULL) {
		DEBUG("The threadpool received as argument is NULL.");
		return -1;
	}
	...
	if ((task = (struct threadpool_task*)threadpool_queue_dequeue(&(pool->free_tasks_queue))) == NULL) {
		DEBUG("Failed to obtain an empty task from the free tasks queue.");
		if (pthread_mutex_unlock(&(pool->free_tasks_mutex))) {
			log_error(errno, "pthread_mutex_unlock");
		}
	...
	if (threadpool_queue_enqueue(&(pool->tasks_queue),task)) {
		DEBUG("Failed to add a new task to the tasks queue.");
		if (pthread_mutex_unlock(&(pool->mutex))) {
			log_error(errno, "pthread_mutex_unlock");
		}
		return -1;
...
void threadpool_free(struct threadpool *pool, int blocking)
{
	...
	if (blocking) {
		...
		if (pthread_create(&thr,NULL,stop_worker_thr_routines_cb,pool)) {
			log_error(errno, "pthread_create");
			DEBUG("Warning! will not be able to free memory allocation. This will cause a memory leak.");
			pool->stop_flag = 1;
		}
	}
}

unsigned int threadpool_get_num_of_threads(struct threadpool *pool)
{
	if (pool == NULL)
		return 0;
	
	return pool->num_of_threads;
}

unsigned int threadpool_get_length_of_tasks_queue(struct threadpool *pool)
{
	struct threadpool_queue *queue;

	if (pool == NULL)
		return 0;
	
	queue = &pool->tasks_queue;
...
unsigned int threadpool_get_length_of_free_tasks_queue(struct threadpool *pool)
{
	struct threadpool_queue *queue;
